...
**The one thing to get right: your controls are parented to `CScrollPanel_GetPanel()`, never to
...
| File | Purpose |
|---|---|
| `CScrollPanel.bi` | Declarations — types, callbacks, constants, function prototypes |
| `CScrollPanel.inc` | Implementation |
| `CVScrollBar.bi` | The vertical scrollbar the control creates and owns |
| `CVScrollBar.inc` | Its implementation |
...
**Include order.** `CScrollPanel.bi` pulls in `CBufferPaint.bi` and `CVScrollBar.bi` for the
...
1. **Parent your controls to `CScrollPanel_GetPanel()`**, never to the control itself.
...
5. **Tell the control how tall the page is** — return a height from the layout callback, or
   call `CScrollPanel_SetContentHeight` / `CScrollPanel_AutoSizeContent`. Left at 0, the page
   is stretched to the viewport and nothing scrolls.
6. **Call `CScrollPanel_Recalc`** after adding, removing, hiding or moving children outside of
   a resize. The control does not watch the page for you.
7. **Keep ownership of any `HFONT` you pass to `CScrollPanel_SetFont`.** The control never
...
dim as HWND hPage = CScrollPanel_GetPanel( ghScrollPanel )
...
`CScrollPanel_AutoSizeContent( h, padding )`, which measures the deepest visible child.
...
`CScrollPanel_Create` returns the **container**. It is an ordinary window handle, not an opaque
...
`CScrollPanel_GetPanel` returns the **page**, and that is the parent for everything you put on
...
runs, so it can never reason about a stale width. Calling `CScrollPanel_SetContentHeight` from
...
sees where they are. `CScrollPanel_ShouldShow( bNeeds, bMouse, bFocus )` is that rule as a pure
...
background — `CScrollPanel_SetColors` pushes it through. The container paints itself in the
...
the colours from `CScrollPanel_SetColors`. That is why one call themes a whole settings page of
...
it moves onto a CScrollPanel: your form's own `WM_COMMAND` handler still sees its controls.
...
CScrollPanel_SetLineHeight`, re-read per message so a Control Panel change takes effect
...
`CScrollPanel_SetPos`, `ScrollBy`, `SetContentHeight` and `EnsureVisible` never fire the scroll
...
- **The control does not watch the page.** Adding, removing, hiding or moving a child outside of
  a resize needs a `CScrollPanel_Recalc` before the scroll range reflects it.
- **`CScrollPanel_SetFont` applies to existing children only.** Controls created afterwards keep
...
- **`CScrollPanel_EnsureVisible` refuses a window that is not a descendant of the page**,
...
| `CScrollPanel_GetPanel( hScp ) as HWND` | The page. **Parent every control you put on the page to this handle**, and lay them out in page coordinates. A control parented to the container instead would sit over the viewport and never scroll. |
| `CScrollPanel_GetScrollBar( hScp ) as HWND` | The owned `CVScrollBar`, for anything this API does not expose. Do not show, hide, move or resize it — the control owns its visibility and its rectangle. |

### The page

| Function | Description |
|---|---|
| `CScrollPanel_SetContentHeight( hScp, nHeight )` | Declares how tall the page is, in pixels. Clamped to a minimum of 0; a no-op when the value is unchanged. Re-clamps the current position, re-sizes the page and re-syncs the scrollbar. **Silent.** Safe to call from inside the layout callback, where it records the value and lets that pass finish rather than starting another. |
| `CScrollPanel_GetContentHeight( hScp ) as integer` | The declared content height. Note this is the *declared* value, which may be less than the page window's actual height when the page is shorter than the viewport. |
| `CScrollPanel_AutoSizeContent( hScp, bottomPadding = 0 ) as integer` | Measures the bottom edge of the deepest **visible** child of the page, adds `bottomPadding`, pushes the result through `SetContentHeight` and returns it. Hidden children are skipped, so a collapsed section leaves no tail of empty page behind it. With no visible children it returns 0 and the padding is not added. |
| `CScrollPanel_Recalc( hScp )` | Forces a full layout pass — the layout callback, the page geometry, the strip, the scrollbar sync — and repaints the page. Call it after adding, removing, hiding or moving children outside of a resize. |
| `CScrollPanel_SetFont( hScp, hFont )` | Sends `WM_SETFONT` to every **existing** child of the page. Call it after building the page. **You keep ownership of the `HFONT`**; the control never deletes it, and the font must outlive the page. |
...
| Function | Description |
|---|---|
| `CScrollPanel_GetPos( hScp ) as integer` | The current scroll offset: the page's first visible row of pixels. The page's top edge in container coordinates is `-GetPos()`. |
| `CScrollPanel_SetPos( hScp, nPosition )` | Moves the page there, clamped to `0 … contentHeight - viewportHeight`. Silent. |
| `CScrollPanel_ScrollBy( hScp, nDelta ) as boolean` | Moves relative to the current position, clamped the same way. Returns TRUE only if the position actually changed — so it reports FALSE at either end. Silent. |
| `CScrollPanel_EnsureVisible( hScp, hChild, nMargin = 0 ) as boolean` | Scrolls the **minimum** distance that brings `hChild` fully into view, with `nMargin` pixels of clearance above and below. A child taller than the viewport is top-aligned instead. Returns FALSE and does nothing if `hChild` is not a descendant of the page, or if no move was needed. Silent, and it never calls `SetFocus`. |
| `CScrollPanel_HandleWheelDelta( hScp, nDelta ) as boolean` | Feeds a wheel delta to the page, for a host that received the gesture itself. Pass the **already-extracted signed** delta: `CScrollPanel_HandleWheelDelta( hScp, cast(short, hiword(wParam)) )`. The cast is not optional — `hiword` yields an unsigned word, so a delta read without it turns −120 into ~65416 and the direction silently inverts. Returns TRUE if the page moved. **This one is not silent**: it is user action, so the scroll callback fires. |
| `CScrollPanel_SetLineHeight( hScp, nHeight )` | One "line" for the wheel, in **unscaled** pixels (DPI-scaled at use). Clamped to a minimum of 1; default 20. One notch = system wheel lines × this. Set it to your row pitch and a notch moves whole rows. |
| `CScrollPanel_SetAutoScrollToFocus( hScp, bEnable )` | ON by default. With it on, focus landing on an off-screen child — from a Tab or from your own `SetFocus` — scrolls that child into view. The scroll happens on a focus **change** only, so deliberately scrolling the focused control away does not yank it back. This scroll *does* fire the scroll callback. |
| `CScrollPanel_GetAutoScrollToFocus( hScp ) as boolean` | The current setting. |

### Geometry and layout

| Function | Description |
|---|---|
| `CScrollPanel_GetViewportHeight( hScp ) as integer` | The container's client height — the visible slice of the page. Read live, never cached. |
| `CScrollPanel_GetPanelWidth( hScp ) as integer` | The page's width: the container's client width minus the scaled strip width, floored at 0. This is the same number the layout callback is handed as `cxPanel`. |
| `CScrollPanel_IsThumbVisible( hScp ) as boolean` | What the reveal rule last decided, i.e. whether the bar window is currently shown. Updated on each poll tick and on every scrollbar sync. |
| `CScrollPanel_ShouldShow( bNeeds, bMouse, bFocus ) as boolean` | The reveal rule itself, as a pure function of three booleans — no window, no mouse, no focus required. `FALSE` unless `bNeeds` and at least one of `bMouse` / `bFocus`. Exposed so a host can predict or assert the behaviour. |

### Appearance

| Function | Description |
|---|---|
| `CScrollPanel_SetColors( hScp, backclr, foreclr )` | Sets the page's fill **and** the colours handed to your controls through `WM_CTLCOLOR*`. When the background changes it also rebuilds the `WM_CTLCOLOR` brush and pushes the new background to the scrollbar, which is what keeps the reserved strip invisible. Repaints the container, and the page with its children. **Call this before `CScrollPanel_SetScrollBarColors`** — it overwrites the bar's background colour whenever the page background changes. |
| `CScrollPanel_SetScrollBarColors( hScp, backclr, foreclr, foreclrhot )` | Forwarded straight to the owned `CVScrollBar`: strip background, thumb, hot thumb. |
| `CScrollPanel_SetScrollBarWidth( hScp, nWidth )` | The reserved strip's width, in **unscaled** pixels (DPI-scaled at use). Clamped to a minimum of 0; default 12. A no-op when unchanged, otherwise it triggers a full layout pass, because the page's width changed with it. |

### Callback registration

| Function | Description |
|---|---|
| `CScrollPanel_SetPaintCallback( hScp, usersub )` | Installs a painter for the page background **instead of** the built-in flat fill. Repaints. |
| `CScrollPanel_SetMessageCallback( hScp, userfunc )` | Installs an observer for messages on the container and the page. |
| `CScrollPanel_SetLayoutCallback( hScp, userfunc )` | Installs the re-flow handler called on every resize. |
| `CScrollPanel_SetScrollCallback( hScp, usersub )` | Installs the handler told when the **user** scrolls. |
...
|---|---|---|---|
| Background | `CScrollPanel_SetColors`, 2nd arg | `BGR(33,37,43)` | The page's fill, the container's fill (visible only in the strip), the `WM_CTLCOLOR*` background, and the brush handed back to `WM_CTLCOLOR*` |
| Foreground | `CScrollPanel_SetColors`, 3rd arg | `BGR(215,218,224)` | The `WM_CTLCOLOR*` text colour for every control on the page |
| Scrollbar background | `CScrollPanel_SetScrollBarColors`, 2nd arg | tracks the page background | The strip behind the thumb |
| Scrollbar thumb | `CScrollPanel_SetScrollBarColors`, 3rd arg | `BGR(53,59,69)` | The thumb, idle |
| Scrollbar thumb, hot | `CScrollPanel_SetScrollBarColors`, 4th arg | `BGR(90,98,112)` | The thumb, hovered or dragged |

The defaults are a dark theme, so a control you never call `CScrollPanel_SetColors` on still
looks right.
...
is rebuilt by `CScrollPanel_SetColors` and deleted when the control is destroyed; you never
touch it.
...
Calling `CScrollPanel_SetContentHeight` from inside this callback is safe: the control is in a
layout pass, and the setter records the value instead of starting another one.

This callback runs on every resize, on `CScrollPanel_Recalc`, and whenever the strip width
changes.
...
| `b` | The page's `CBufferPaint` for this repaint (borrowed, not owned) |
...
`CScrollPanel_GetPos( hScrollPanel )` already equals `newPos`.

It does not fire for `CScrollPanel_SetPos`, `ScrollBy`, `SetContentHeight` or `EnsureVisible`,
...
|---|---:|---|
| `CSCROLLPANEL_LINE_HEIGHT` | 20 | Default wheel line height in unscaled pixels, DPI-scaled at use. Change it per instance with `CScrollPanel_SetLineHeight`. |
| `CVSCROLL_DEFAULT_WIDTH` | 12 | Default strip width in unscaled pixels, DPI-scaled at use. From `CVScrollBar.bi`. Change it per instance with `CScrollPanel_SetScrollBarWidth`. |
| `CSCROLLPANEL_REVEAL_MS` | 100 | The reveal poll interval, in milliseconds. |
...
CScrollPanel **creates and owns a `CVScrollBar`**, which is why `CVScrollBar.bi` / `.inc` are
required files rather than optional ones. The bar's rectangle, visibility, range and position
all belong to the control; `CScrollPanel_GetScrollBar` exists for the things this API does not
expose, such as installing a custom bar painter. Do not show, hide, move or resize it yourself.

`CBufferPaint` is the drawing surface both windows paint through, and the object a paint
